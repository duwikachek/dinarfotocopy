...
type SettingsState = typeof dummySettings & { store_whatsapp: string };
...
export default function AdminPengaturanPage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      // Save each setting key individually via server action
      const entries: [string, unknown][] = [
        ["store_name", settings.store_name],
        ["store_tagline", settings.store_tagline],
        ["store_address", settings.store_address],
        ["store_whatsapp", settings.store_whatsapp],
        ["store_operational_hours", settings.store_operational_hours],
        ["map_embed_url", settings.map_embed_url],
        ["notify_admin_on_new_order", settings.notify_admin_on_new_order],
        ["bulk_discount_rules", settings.bulk_discount_rules],
      ];

      const results = await Promise.all(
        entries.map(([key, value]) => updateStoreSettingsAction(key, value))
      );

      const failed = results.find((r) => !r.success);
      if (failed) {
        showToast(failed.error || "Gagal menyimpan beberapa pengaturan.", "error");
      } else {
  ...
  const inputCls = "w-full h-10 px-3 text-sm font-medium rounded-lg border border-[hsl(220,13%,85%)] bg-white text-[hsl(224,12%,12%)] placeholder:text-[hsl(220,10%,65%)] focus:outline-none focus:border-[hsl(224,12%,12%)] focus:ring-2 focus:ring-amber-400/20 transition-colors";
  const textareaCls = "w-full px-3 py-2 text-sm font-medium rounded-lg border border-[hsl(220,13%,85%)] bg-white text-[hsl(224,12%,12%)] placeholder:text-[hsl(220,10%,65%)] focus:outline-none focus:border-[hsl(224,12%,12%)] focus:ring-2 focus:ring-amber-400/20 resize-none transition-colors";
  ...
  const fields: Array<{
    ...
    { key: "store_whatsapp", label: "No. WhatsApp Toko (format internasional)", type: "text", placeholder: "628123456789" },
