...
import { ArrowLeft, MessageCircle, Send, CheckCircle, Clock, Package, Printer, X } from "lucide-react";
...
export default function AdminDetailPesananPage({ params }: Props) {
  ...
  const handleStatusChange = (newStatus: OrderStatus) => {
    ...
    startStatus(async () => {
      ...
      if (res.success) {
        showToast(`Status diubah ke "${orderStatusConfig[newStatus].label}"`);
      } else {
  ...
  const handleSendNotif = async () => {
    ...
    setSending(false);
    if (res.success) {
      showToast("Notifikasi WhatsApp berhasil dikirim!");
    } else {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      ...
      {toast.msg && (
        <div
          className={`fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 rounded-xl text-white text-sm font-medium shadow-md animate-fade-in ${
            toast.type === "error" ? "bg-red-600" : "bg-[hsl(224,12%,12%)]"
          }`}
        >
          {toast.type === "error" ? (
      ...
      <div className="flex items-center gap-3">
        <Link
          href="/admin/pesanan"
          className="flex items-center gap-1.5 text-sm text-[hsl(220,10%,46%)] hover:text-[hsl(224,12%,12%)] transition-colors"
        >
          ...
          <div className="p-5 rounded-xl bg-white border border-[hsl(220,13%,91%)]">
            ...
            <div className="flex flex-col gap-2 text-sm">
              <div className="flex justify-between">
                <span className="text-[hsl(220,10%,46%)]">Nama</span>
                <strong className="text-[hsl(224,12%,12%)]">{order.customerName}</strong>
              </div>
              ...
              <div className="flex justify-between">
                <span className="text-[hsl(220,10%,46%)]">Pengambilan</span>
                <span className="text-[hsl(224,12%,12%)]">{order.pickupMethod === "pickup" ? "Ambil di toko" : "Antar"}</span>
              </div>
              {order.customerNote && (
                  ...
                  <tr key={item.id}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {item.itemType === "service" ? (
                          <Printer className="w-3.5 h-3.5 text-[hsl(220,10%,55%)]" strokeWidth={1.5} />
                        ) : (
                          <Package className="w-3.5 h-3.5 text-[hsl(220,10%,55%)]" strokeWidth={1.5} />
                        )}
                    ...
                    <td className="px-4 py-3 font-bold tabular-nums text-[hsl(224,12%,12%)]">
                      ...
                      {item.discountPercent > 0 && (
                        <span className="ml-1 text-xs text-green-600 font-normal">(-{item.discountPercent}%)</span>
                      )}
                    </td>
                  </tr>
                ))}
                  ...
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                      idx === order.statusLogs.length - 1
                        ? "bg-[hsl(224,12%,12%)]"
                        : "bg-[hsl(220,14%,96%)] border border-[hsl(220,13%,91%)]"
                    }`}
                  >
                    {idx === order.statusLogs.length - 1 ? (
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              {statusFlow.map((s, idx) => (
                <button
                  ...
                  disabled={statusPending || (s === "completed" && status !== "ready")}
                  className={cn(
              ...
              <button
                ...
                disabled={statusPending || status === "cancelled" || status === "completed"}
                className="flex items-center justify-between px-3 py-2 rounded-lg text-sm border border-red-200 text-red-600 hover:bg-red-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {status === "cancelled" ? (
                  <>
              ...
              {statusPending && (
                <p className="text-xs text-center text-[hsl(220,10%,55%)] animate-pulse">Menyimpan status...</p>
              )}
            </div>
          </div>
          ...
          <div className="p-5 rounded-xl bg-white border border-[hsl(220,13%,91%)]">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Catatan untuk tim... (akan disimpan saat ubah status)"
              className="w-full text-sm px-3 py-2 rounded-lg border border-[hsl(220,13%,91%)] bg-white focus:outline-none focus:border-[hsl(224,12%,12%)] resize-none transition-colors"
            />
            <p className="text-[10px] text-[hsl(220,10%,60%)] mt-1">Catatan disimpan otomatis saat mengubah status.</p>
          </div>
          ...
          <div className="p-5 rounded-xl bg-white border border-[hsl(220,13%,91%)]">
            ...
            >
              {sending ? (
                ...
                <><Send className="w-4 h-4" strokeWidth={1.5} />Kirim Notifikasi WA</>
              )}
            </button>
