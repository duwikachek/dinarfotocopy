...
import type { ServiceWithCategory } from "@/db/queries";
...
export default function DetailLayananClient({ service }: { service: ServiceWithCategory }) {
  const addServiceItem = useCartStore((s) => s.addServiceItem);
  const Icon = categoryIcons[service.categorySlug] ?? Printer;

  const handleAddToCart = (variant: ServiceWithCategory["variants"][0]) => {
    addServiceItem({
      type: "service",
      ...
      quantity: 1,
      isBulkEligible: (service as any).isBulkDiscountEligible ?? true,
    });
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="mb-8">
            ...
            {(service as any).isBulkDiscountEligible && (
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-20 p-5 rounded-xl border border-[hsl(220,13%,91%)] bg-white">
            ...
            <p className="text-xs text-[hsl(220,10%,55%)] mb-5">
              per {service.unit} · {service.variants?.length || 0} varian tersedia
            </p>
